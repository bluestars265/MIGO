...
def show_settings(parent, settings, save_callback):
    ...
    # Java路径
    tk.Label(win, text=lgag["language"][lgag["user_choice"]]["java_exe_path"], bg=win.bg).place(x=20, y=20)
    ...
    # 替换为样式按钮
    btn_java = create_gradient_button(win, text=lgag["language"][lgag["user_choice"]]["apply"], command=save_java, width=80, height=30)
    btn_java.place(x=510, y=40)

    # 服务器jar路径
    tk.Label(win, text=lgag["language"][lgag["user_choice"]]["server_jar_path"], bg=win.bg).place(x=20, y=90)
    ...
    btn_jar = create_gradient_button(win, text=lgag["language"][lgag["user_choice"]]["apply"], command=save_jar, width=80, height=30)
    btn_jar.place(x=510, y=110)

    # 关闭按钮
    btn_close = create_gradient_button(win, text=lgag["language"][lgag["user_choice"]]["close"], command=win.destroy, width=80, height=30)
